...
namespace hdl_graph_slam
{
  ...
  GraphSLAM::GraphSLAM(const std::string &solver_type)
  {
    ...
    // 왜 스마트포인터로 생성을 한 다음에 캐스팅해서 원시 포인터를 추출하는건지 궁금
    // graph.reset(new g2o::SparseOptimizer());에서 관리되는 graph라는 놈은 클래스 멤버 변수고
    // *graph는 원시 포인터라 이름만 같고 역할은 다름
    // 그래서 이 structure 안에서 생성한 *graph란 놈은 여기서만 사용하고 없어지는...
